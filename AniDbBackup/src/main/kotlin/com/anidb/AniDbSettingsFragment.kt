...
class AniDbSettingsFragment(private val plugin: Plugin) : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(
        ...
        bypassBtn.setOnClickListener {
            val dialog = CloudflareWebViewDialog(
                targetUrl = "https://anidb.app",
                onFinished = { saved ->
                    if (saved) bypassBtn.text = "✅ CF Cookies Saved – Refresh"
                }
            )
            dialog.show(parentFragmentManager, "anidb_cf_bypass")
        }
        // Update button label to show current cookie status
        val cfCookies = AniDbPlugin.cfCookies
            ...
            context?.let { ctx ->
                AlertDialog.Builder(ctx)
                    ...
                    .setPositiveButton("Clear") { _, _ ->
                        ...
                        val host = AniDbPlugin.cfCookieHost
                        if (host.isNotBlank()) {
                        ...
                        // Clear plugin store
                        AniDbPlugin.cfCookies    = ""
                        AniDbPlugin.cfUserAgent  = ""
                        AniDbPlugin.cfCookieHost = ""
