...
internal fun String.cleanNd(): String {
    return replace("\\/", "/")
        .replace("\\u0026", "&")
        .replace("&amp;", "&")
        .replace("&quot;", "\"")
...
internal fun String.cleanTitleNd(): String {
    return cleanNd()
        .replace(Regex("""(?i)\s*[-|]\s*(NontonDrama|Nonton Drama|NontonDrama|Nonton Drama).*$"""), "")
...
internal fun String?.absUrlNd(baseUrl: String): String? {
    val raw = this?.cleanNd()?.trim()?.takeIf { it.isNotBlank() } ?: return null
...
internal fun String.urlEncodeNd(): String = URLEncoder.encode(trim(), "UTF-8")

internal fun String.sameHostNd(baseUrl: String): Boolean {
    return runCatching {
...
internal fun String.sameRootDomainNd(baseUrl: String): Boolean {
    return runCatching {
        val baseHost = URI(baseUrl).host.orEmpty().removePrefix("www.")
        val host = URI(this).host.orEmpty().removePrefix("www.")
        host.equals(baseHost, ignoreCase = true) ||
            (baseHost.endsWith("nontondrama.blog", true) && host.endsWith("nontondrama.blog", true))
    }.getOrDefault(false)
}

internal fun String.isVideoUrlNd(): Boolean {
    val lower = lowercase()
    return lower.contains(".m3u8") || lower.contains(".mp4") || lower.contains(".webm") || lower.contains(".mpd") || lower.contains(".txt")
}

internal fun String.isSubtitleUrlNd(): Boolean {
    val lower = lowercase()
    return lower.contains(".srt") || lower.contains(".vtt") || lower.contains(".ass")
}

internal fun String.isNoiseUrlNd(): Boolean {
    val lower = lowercase()
...
internal fun String.posterCandidateScoreNd(): Int {
    val lower = lowercase()
    var score = 0
...
internal fun String.qualityNd(): Int {
    val value = lowercase()
...
internal fun decodeBase64Nd(raw: String): String? {
