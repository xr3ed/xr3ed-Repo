...
object GarasiFilm21Extractor {
    ...
    suspend fun load(pageUrl: String, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        suspend fun resolve(raw: String?, referer: String = pageUrl, depth: Int = 0) {
            val url = raw.absUrlGf21(referer) ?: return
            if (url.isNoiseUrlGf21()) return
            if (!visited.add(url)) return

            when {
                url.isSubtitleUrlGf21() -> {
            ...
            val response = runCatching {
                app.get(
                    url = url,
                    headers = GarasiFilm21Network.baseHeaders + mapOf("Referer" to referer),
                    referer = referer,
                    timeout = 15000L
                )
            }.getOrNull() ?: return
            ...
            val body = runCatching { response.text.cleanGf21() }.getOrNull() ?: return
            for (candidate in collectCandidates(body, url)) {
                runCatching { resolve(candidate, url, depth + 1) }
            }
        }

        val html = GarasiFilm21Network.getText(pageUrl, GarasiFilm21Provider.DEFAULT_MAIN_URL).cleanGf21()
        val document = GarasiFilm21Parser.parseDocumentFromHtml(html, pageUrl)
    ...
    private suspend fun extractMinochinos(iframeUrl: String, emit: (ExtractorLink) -> Unit) {
        ...
        for (tryUrl in embedUrls) {
            iframeHtml = runCatching {
                app.get(
                    url = tryUrl,
                    headers = GarasiFilm21Network.baseHeaders + mapOf("Referer" to GarasiFilm21Provider.DEFAULT_MAIN_URL),
                    referer = GarasiFilm21Provider.DEFAULT_MAIN_URL
                ).text.cleanGf21()
            }.getOrNull().orEmpty()
        ...
        if (!hash.isNullOrBlank() && fileCode.isNotBlank()) {
            val viewUrl = "https://minochinos.com/dl?op=view&file_code=$fileCode&hash=$hash&embed=1&referer=tv9.gf21.fun&adb=0&hls4=1"
        ...
        Regex("""["']hls\d*["']\s*:\s*["']([^"']+)["']""", RegexOption.IGNORE_CASE)
            .findAll(unpacked)
            .mapNotNull { it.groupValues.getOrNull(1)?.absUrlGf21(iframeUrl) }
        ...
        Regex("""file\s*:\s*links\.(hls\d*)""", RegexOption.IGNORE_CASE)
            ...
            .forEach { key ->
                Regex("""["']$key["']\s*:\s*["']([^"']+)["']""", RegexOption.IGNORE_CASE)
                    ...
                    ?.groupValues
                    ?.getOrNull(1)
                    ?.absUrlGf21(iframeUrl)
                    ?.let(hlsCandidates::add)
            }

        collectCandidates(unpacked, iframeUrl)
            .filter { it.isVideoUrlGf21() }
            .forEach(hlsCandidates::add)

        hlsCandidates
            .filterNot { it.isNoiseUrlGf21() }
    ...
    private suspend fun extractGdrivePlayer(playerUrl: String, referer: String, emit: (ExtractorLink) -> Unit) {
        val playerHtml = runCatching {
            app.get(
                url = playerUrl,
                headers = GarasiFilm21Network.baseHeaders + mapOf(
    ...
    private fun collectGdriveHlsCandidates(html: String, baseUrl: String): Set<String> {
        ...
        Regex("""((?:https?:)?//gdriveplayer\.(?:io|to|me)/hlsplaylist\.php[^\s"'<>),;]+)""", RegexOption.IGNORE_CASE)
            .findAll(clean)
            .mapNotNull { match -> match.groupValues.getOrNull(1)?.cleanGdriveCandidate()?.absUrlGf21(baseUrl) }
            .forEach(out::add)

        Regex("""(?:^|["'=:(\s])(/hlsplaylist\.php[^\s"'<>),;]+)""", RegexOption.IGNORE_CASE)
            .findAll(clean)
            .mapNotNull { match -> match.groupValues.getOrNull(1)?.cleanGdriveCandidate()?.absUrlGf21(gdriveBase) }
            .forEach(out::add)
        ...
        if (!sValue.isNullOrBlank() && !idhlsValue.isNullOrBlank()) {
            val idhls = idhlsValue.cleanGf21().let { value -> if (value.endsWith(".m3u8", true)) value else "$value.m3u8" }
    ...
    private fun decodeGdriveXor(key: String?, encoded: String?): String? {
        ...
        return builder.toString().cleanGf21()
    }
    ...
    fun collectCandidates(html: String, baseUrl: String): Set<String> {
        ...
        fun collectFrom(text: String, currentBase: String = baseUrl) {
            val normalized = text.cleanGf21()
            candidatePatterns.forEach { pattern ->
                pattern.findAll(normalized).forEach { match ->
                    match.groupValues.getOrNull(1)?.absUrlGf21(currentBase)?.let(out::add)
                }
            }
        }
        ...
        Regex("""atob\(["']([^"']+)["']\)""", RegexOption.IGNORE_CASE)
            .findAll(clean)
            .mapNotNull { decodeBase64Gf21(it.groupValues[1]) }
        ...
        return out.filterNot { it.isNoiseUrlGf21() }.toCollection(linkedSetOf())
    }
    ...
    private suspend fun emitGdriveHls(url: String, emit: (ExtractorLink) -> Unit) {
        emit(
            newExtractorLink("GarasiFilm21", "GdrivePlayer HLS", url, ExtractorLinkType.M3U8) {
                this.referer = ""
                this.quality = url.qualityGf21()
                this.headers = mapOf(
    ...
    private suspend fun emitDirect(url: String, referer: String, emit: (ExtractorLink) -> Unit, sourceName: String = "GarasiFilm21") {
        val type = if (url.contains(".m3u8", true) || url.contains(".txt", true)) ExtractorLinkType.M3U8 else INFER_TYPE
        emit(
            newExtractorLink("GarasiFilm21", sourceName, url, type) {
    ...
    private fun String.toGdriveQueryValue(): String {
        val value = cleanGf21()
        return if (value.contains("%")) value else value.urlEncodeGf21()
    }
